...
<template>
  <div class="app-container calendar-list-container">
    ...
    >
      ...
      <el-table-column align="center" label="详情图">
        <template slot-scope="scope">
          <el-button
            type="primary"
            icon="el-icon-view"
            @click="beforeView(scope.row.description)"
          >查看</el-button>
        </template>
      </el-table-column>
...
import * as api from "@/service/goverment";
export default {
  mixins: [tableMixins],
  name: "goverment",
  data() {
    return {
      ...
      rules: {
        ...
        priority: [{ required: true, message: "请输入顺序", trigger: "blur" }],
        description: [
  ...
  methods: {
    ...
    handleSuccess(res, file, fileList) {
      this.addQuery.description = res.data;
    },
    handleRemove(file, fileList) {
      this.addQuery.description = "";
    },
    handlePreview() {
      this.imgUrl = url;
    ...
    beforOpenDialog(type, item) {
      if (type === 1) {
        ...
        this.resetForm();
        const { id, description, title, priority } = item;
        this.addQuery = { ...this.addQuery, id, description, title, priority };
        this.fileList = [{ name: "", url: description }];
      }
  ...
  computed: {
    diglogTitle() {
      return this.dialogType === 1 ? "新增公告" : "编辑公告";
    }
  }
};
</script>
